...
for naic in naics_df:
    # Read naic_subset data from CSV files
...
# Load wage_percentiles_02to16 data
wage_percentiles_02to16 = pd.read_csv(r'./data/intermediate/sector_wages/industry_weighted_percs_02to16.csv', header=0)
wage_percentiles_02to16['naics'] = wage_percentiles_02to16['naics'].astype(str)

# Merge data_naics with wage_percentiles_02to16
for naic, df in data_naics.items():
    data_naics[naic] = data_naics[naic].merge(wage_percentiles_02to16, on=['naics','year'], how='left')
    data_naics[naic]['fips'] = data_naics[naic]['fipstate'] + data_naics[naic]['fipscty']

# Create county_percentiles DataFrame
...
# Iterate over data_naics to calculate county_percentiles
h = -1
...
county_percentiles['fipstate'] = county_percentiles['fips'].str[:2]
county_percentiles['fipscty'] = county_percentiles['fips'].str[2:]

county_percentiles

# Save county_percentiles to CSV file
county_percentiles.to_csv(r'./data/intermediate/sector_wages/county_percentiles.csv',sep=',',index=False)